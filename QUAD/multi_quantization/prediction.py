...
def joint_codebook_loss(predictor: Tensor,
    ...
    if len(codebook_indexes.shape) == 3:
        print("length of codebook_indexes.shape is ", len(codebook_indexes.shape))
        ...
        return torch.nn.functional.cross_entropy(logprobs.reshape(-1, codebook_size), codebook_indexes.reshape(-1), ignore_index=ignore_index, reduction=reduction)
    # multiple teachers
...
def masked_cross_entropy(logits, processed_targets, mask, reduction='mean'):
    """
    Args:
        logits: [N,256]
