...
def train_quad(epoch, train_loader, model, criterion, optimizer, opt, get_codebook_indexes):
    """quad training"""
    # Create a GradScaler for mixed precision training
    #scaler = amp.GradScaler()
    model.train()
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        # ===================forward=====================
        output, codebook_loss = model(input, codebook_indexes=get_codebook_indexes(idx, opt), target=target)
        loss = criterion(output, target)
        # print(f"Batch {idx}:")
        # print(f"  Classification loss: {loss.item():.6f}")
        # print(f"  Codebook loss: {codebook_loss.item():.6f}")
        # print(f"  Loss ratio (cls/codebook): {loss.item()/codebook_loss.item():.6f}")
        loss += codebook_loss
        losses.update(loss.item(), input.size(0))
